...
struct clip_graph {
    ...
    // --- Новый backend: StarVector ---
    ggml_cgraph * build_starvector() {
        // 1. Получаем входные патчи (Conv2d → flatten → pos embd)
        ggml_tensor * inp = build_inp();
        // 2. Добавляем CLS token (если есть)
        if (model.class_embedding) {
            inp = ggml_concat(ctx0, inp, model.class_embedding, 1);
        }
        ...
        // 4. ViT-энкодер (N слоёв)
        ggml_tensor * cur = build_vit(
            inp, n_patches + 1, // +1 если CLS
            NORM_TYPE_NORMAL,
            hparams.ffn_op,
            nullptr,
            nullptr);
        // 5. LayerNorm (post_ln_w/b)
        if (model.post_ln_w) {
            cur = build_norm(cur, model.post_ln_w, model.post_ln_b, NORM_TYPE_NORMAL, eps, -1);
        }
        // 6. Adapter/MLP-head (если есть)
        if (model.projection) {
            cur = ggml_mul_mat(ctx0, model.projection, cur);
        }
        // 7. Собираем граф
...
bool clip_image_preprocess(struct clip_ctx * ctx, const clip_image_u8 * img, struct clip_image_f32_batch * res_imgs) {
    ...
    if (clip_is_starvector(ctx)) {
        // 1. RGBA → RGB (белый фон)
        clip_image_u8_ptr rgb_img(clip_image_u8_init());
        *rgb_img = *img;
        if (img->buf.size() == img->nx * img->ny * 4) {
            rgb_img->buf.resize(img->nx * img->ny * 3);
            for (int i = 0; i < img->nx * img->ny; ++i) {
                float alpha = img->buf[i*4+3] / 255.0f;
                for (int c = 0; c < 3; ++c) {
                    rgb_img->buf[i*3+c] = (uint8_t)(img->buf[i*4+c] * alpha + 255 * (1.0f - alpha));
                }
            }
        }
        // 2. Padding до квадрата (белый фон)
        int max_dim = std::max(rgb_img->nx, rgb_img->ny);
        clip_image_u8_ptr padded_img(clip_image_u8_init());
        padded_img->nx = max_dim;
        padded_img->ny = max_dim;
        padded_img->buf.resize(3 * max_dim * max_dim, 255);
        int pad_x = (max_dim - rgb_img->nx) / 2;
        int pad_y = (max_dim - rgb_img->ny) / 2;
        for (int y = 0; y < rgb_img->ny; ++y) {
            for (int x = 0; x < rgb_img->nx; ++x) {
                for (int c = 0; c < 3; ++c) {
                    padded_img->buf[3 * ((y + pad_y) * max_dim + (x + pad_x)) + c] = rgb_img->buf[3 * (y * rgb_img->nx + x) + c];
                }
            }
        }
        // 3. Resize (bicubic) до ctx->vision_model.hparams.image_size
        int target_size = ctx->vision_model.hparams.image_size;
        clip_image_u8_ptr resized_img(clip_image_u8_init());
        image_manipulation::bicubic_resize(*padded_img, *resized_img, target_size, target_size);
        // 4. ToTensor + нормализация (mean/std)
        clip_image_f32_ptr norm_img(clip_image_f32_init());
        norm_img->nx = target_size;
        norm_img->ny = target_size;
        norm_img->buf.resize(3 * target_size * target_size);
        for (int i = 0; i < target_size * target_size; ++i) {
            for (int c = 0; c < 3; ++c) {
                float v = resized_img->buf[3*i+c] / 255.0f;
                norm_img->buf[3*i+c] = (v - STARVECTOR_MEAN[c]) / STARVECTOR_STD[c];
            }
        }
        res_imgs->entries.push_back(std::move(norm_img));
        return true;
    }
...
// Функция-детектор для StarVector
bool clip_is_starvector(const struct clip_ctx * ctx) {
    return ctx->proj_type == PROJECTOR_TYPE_STARVECTOR;
}

// Значения mean/std для StarVector (как в оригинале)
const float STARVECTOR_MEAN[3] = {0.48145466f, 0.4578275f, 0.40821073f};
const float STARVECTOR_STD[3]  = {0.26862954f, 0.26130258f, 0.27577711f};
