...
class ModelBase:
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        del bid  # unused
        # Ранее vision-тензоры пропускались, теперь сохраняем все тензоры без фильтрации
...
@ModelBase.register("StarVectorForCausalLM")
class StarVectorModel(TextModel):
    model_arch = gguf.MODEL_ARCH.STARVECTOR

    def set_vocab(self):
        # Use SentencePiece or fallback to GPT2 vocab if needed
        try:
            self._set_vocab_sentencepiece()
        except FileNotFoundError:
            self._set_vocab_gpt2()
    ...
    def set_gguf_parameters(self):
        hparams = self.hparams
        self.gguf_writer.add_context_length(hparams.get("max_position_embeddings", 8192))
        self.gguf_writer.add_embedding_length(hparams["hidden_size"])
        self.gguf_writer.add_block_count(hparams["num_hidden_layers"])
        self.gguf_writer.add_feed_forward_length(hparams["hidden_size"] * 4)  # StarCoder/StarVector convention
        self.gguf_writer.add_head_count(hparams["num_attention_heads"])
        self.gguf_writer.add_head_count_kv(hparams.get("num_kv_heads", 4))
        self.gguf_writer.add_file_type(self.ftype)
        self.gguf_writer.add_vocab_size(hparams["vocab_size"])
        self.gguf_writer.add_vision_has_vision_encoder(True)
        # Vision encoder params
        self.gguf_writer.add_vision_image_size(hparams.get("image_size", 224))
        # Optionally add more vision params if needed
        # Preprocessor config
        try:
            with open(self.dir_model / "preprocessor_config.json", "r", encoding="utf-8") as f:
                preproc = json.load(f)
            self.gguf_writer.add_vision_image_mean(preproc.get("mean", [0.48145466, 0.4578275, 0.40821073]))
            self.gguf_writer.add_vision_image_std(preproc.get("image_std", [0.26862954, 0.26130258, 0.27577711]))
        except Exception:
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        del bid  # unused
        # Map StarVector tensor names to GGUF, save all tensors without filtering
        return [(self.map_tensor_name(name), data_torch)]
...
class LazyTorchTensor(gguf.LazyBase):
    ...
    def from_safetensors_slice(cls, st_slice: Any) -> Tensor:
        ...
        # Исправление: обработка 0-dim тензоров
        if len(shape) == 0:
